...
public class SignalHelper {
	...
	public static class SignalGenerator {
		...
		public SignalGenerator(int frequency, int N){
		...
		public int read(byte[] audioData, int frequency, double samplingRate) {
			...
			for(int i = 0; i < L; i++){
				double temp = (double)(2*Constants.PI*frequency*((double)i*T));
				short s = (short)(((double)(32767.0F))*Math.sin(temp));
				audioData[2*i] = (byte)(s & 0xFF);
				audioData[2*i+1] = (byte)((s >> 8) & 0xFF);
			}
		...
		public int read(byte[] audioData, int frequency, double samplingRate, boolean addNoise) {
			...
			for(int i = 0; i < L; i++){
				double temp = (double)(2*Constants.PI*frequency*((double)i*T));
				temp = ((((double)(32767.0F/2))*Math.sin(temp)) + (((double)(32767.0F/2))*Math.sin(2*temp)));
				if(addNoise){
		...
		public int read(byte[] audioData, int frequency, double samplingRate, boolean twoSinoids, boolean addNoise) {
