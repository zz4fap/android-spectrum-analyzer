...
public class SignalHelper {
    ...
    static public int[] getTestSignalWithSpecificFrequency(double freq,
            int numberOfFFTPoints, double sampleRateInHz) {
        signal = new int[numberOfFFTPoints / 2];
    ...
    public static class SignalGenerator {
        ...
        public static int read(byte[] audioData, int frequency,
                double samplingRate, boolean twoSinoids, boolean addNoise) {

            double T = (1 / samplingRate); // Sample time
            int L = (int) samplingRate; // Length of signal - it will always be
                                        // equal to 1 [s] once L is equal to the
                                        // sampling frequency.
            short s;
            double temp;
            ...
            for (int i = 0; i < L; i++) {
                ...
                if (twoSinoids) {
                    temp = temp
                            + (((double) (32767.0F / 2)) * Math.sin(2 * arg));
                }
                // add noise to the signal
                if (addNoise) {
                    s = (short) (temp + (double) ((32767.0F / 4) * Math
                            .random()));
                } else {
                    s = (short) temp;
                }
        ...
        public static int read(byte[] audioData, int frequency,
                int numberOfBytesToRead, double samplingRate,
                boolean twoSinoids, boolean addNoise) {
